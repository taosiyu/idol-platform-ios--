...
class MagazineListCtr: BaseCtr {

    //数据源
    private var dataSource:TableViewTwoCellDataSource<MagazineListCtr>!
    ...
    fileprivate func getDataListInfo(isFoot:Bool=false){
        ...
        if !isFoot {
            HttpClient_Alamofire.magazineList(params: dic, success: { (dataObjc) in
...
extension MagazineListCtr:TableViewDataTwoCellSourceDelegate{
...
extension MagazineListCtr:UITableViewDelegate {
