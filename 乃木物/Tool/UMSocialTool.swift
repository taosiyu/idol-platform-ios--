...
class UMSocialTool: NSObject {
    ...
    //MARK:微博分享
    static func shareSINA(url:String,ctr:UIViewController){
        let messageObjc = UMSocialMessageObject()
        ...
        UMSocialManager.default().share(to: UMSocialPlatformType.sina, messageObject: messageObjc, currentViewController: ctr) { (data, error) in
            if ((error) != nil) {
                print("************Share fail with error \(error)*********")
            }else{
                if let resp = data as? UMSocialShareResponse {
    ...
    //MARK:微博分享
    static func shareWECHAT(url:String,ctr:UIViewController){
        let messageObjc = UMSocialMessageObject()
        ...
        UMSocialManager.default().share(to: UMSocialPlatformType.wechatSession, messageObject: messageObjc, currentViewController: ctr) { (data, error) in
            if ((error) != nil) {
                print("************Share fail with error \(error)*********")
            }else{
                if let resp = data as? UMSocialShareResponse {
